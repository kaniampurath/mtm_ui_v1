...
export async function createWorkspace(root, session = {}) {
  ...
  async function hydrateHomeCommandCenter(force = false) {
    ...
    if (!host) return;
    try {
      const data = await api(`/api/home/hierarchy${force ? "?refresh=1" : ""}`, null, "GET");
      if (data.warming) {
        host.innerHTML = `<div class="home-cache-loading"><strong>Market hierarchy cache is warming</strong><p>${e(data.message || "Preparing hierarchy data.")}</p><div class="cache-progress-track"><span style="--w:45%"></span></div></div>`;
        setTimeout(() => hydrateHomeCommandCenter(false), 8000);
        return;
      }
      host.innerHTML = homeCommandCenterTemplate(data);
  ...
  function homeCommandCenterTemplate(data = {}) {
